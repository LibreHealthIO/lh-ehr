...
<html>
  ...
  <body class="body_top">
    ...
    <div style='float: left; margin-right: 5px'>
      <?php if($editlid) {?><!--Edit the disclosures-->
    ...
    <form NAME="disclosure_form" METHOD="POST" ACTION="disclosure_full.php" target='_parent' onsubmit='return top.restoreSession()'>
      ...
      <table border=0 cellpadding=3 cellspacing=0 align='center'>
        ...
        <tr>
          ...
          <td>
            <!--retrieve disclosures from extended_log table for modifications--> 
            <?php 
              if($editlid){
        ...
        <tr>
          ...
          <td class='text'>
            <?php 
              if($editlid){
                  ?> <input type=entry name=recipient_name size=20 value="<?php echo htmlspecialchars($recipient_name,ENT_QUOTES); ?>">
          </td>
          <?php
            }else
            {?>
        ...
        <tr>
          <td>
            <span class=text><?php echo htmlspecialchars(xl('Description of the Disclosure'),ENT_NOQUOTES); ?>:</span>
          </td>
          <?php if($editlid) 
            {
            ?>
