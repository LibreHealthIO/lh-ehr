  ...
  this.validateFormData = function () {   
	...
	this.Save = function() {
      ...
      noteFlat = self.ConvertForSave();
			...
			self.ErrorAlertSet('Notes cannot be saved, document finalized.');
		}
	};
	...
	this.Finalize = function() {
		if(self.Note.Finalized.Finalized() == null) {
			...
			self.ErrorAlertSet('Notes cannot be finalized, already finalized.');
		}
	}
	this.Unfinalize = function() {
		if(self.Note.Finalized.Finalized() != null) {
			...
			self.ErrorAlertSet('Notes cannot be unfinalized, not finalized.');
		}
	};
