...
if (!empty($_POST['bn_save'])) {
  ...
  if (is_uploaded_file($_FILES['form_image']['tmp_name']) && $_FILES['form_image']['size']) {
    ...
    $mime = finfo_file($finfo, $_FILES['form_image']['tmp_name']);
    finfo_close($finfo);
  ...
  if (is_uploaded_file($_FILES['form_education']['tmp_name']) && $_FILES['form_education']['size']) {
    ...
    if ($mime == "application/pdf") {
      ...
      if (!move_uploaded_file($tmp_name, $educationpath)) {
        ...
        die(htmlspecialchars(xl('the file you have uploaded is not a PDF file')));
    } 
  }
...
?></textarea>
 ...
 <tr>
  <td valign='top' class='detail' nowrap>
   ...
   <input type="file" name="form_image" size="40" accept="image/*" />&nbsp;
...
?>
 ...
 <tr>
  <td valign='top' class='detail' nowrap>
   ...
   <input type="file" name="form_education" size="40"  accept="application/pdf"/>&nbsp;
